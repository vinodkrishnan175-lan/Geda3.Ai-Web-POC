...
def _safe_hours(diff: Optional[timedelta]) -> Optional[float]:
    if diff is None:
        return None
    return round(diff.total_seconds() / 3600.0, 4)
...
def standardize_gate(gate_raw: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Optional[str]]]:
    ...
    for i in range(len(df)):
        ...
        gate_in_source = "Workshop_In" if win_dt[i] is not None else ("Reporting" if rep_dt[i] is not None else None)
        base_gate_in_dt = win_dt[i] if win_dt[i] is not None else rep_dt[i]
        timeline_anchor = base_gate_in_dt.date() if base_gate_in_dt is not None else (
            rep_dates[i] or win_dates[i] or wout_dates[i]
        )
        ...
        rows.append(
            {
                ...
                "__reporting_dt__": rep_dt[i],
                "__workshop_in_dt__": win_dt[i],
                "__workshop_out_dt__": wout_dt[i],
...
def standardize_system(sys_raw: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Optional[str]]]:
    ...
    for i in range(len(df)):
        ...
        timeline_anchor = gate_dt[i].date() if gate_dt[i] is not None else (rep_dt[i].date() if rep_dt[i] is not None else (bill_dt[i].date() if bill_dt[i] is not None else None))
        rows.append(
            {
                "__sys_idx__": i,
                ...
                "__system_gate_in_dt__": gate_dt[i],
                "__system_reporting_dt__": rep_dt[i],
                "__system_bill_dt__": bill_dt[i],
...
def _candidate_time_anchor(gate_row: pd.Series) -> Optional[datetime]:
    return gate_row["__base_gate_in_dt__"] if gate_row["__base_gate_in_dt__"] is not None else gate_row["__reporting_dt__"]


def _system_time_anchor(sys_row: pd.Series) -> Optional[datetime]:
    return sys_row["__system_gate_in_dt__"] if sys_row["__system_gate_in_dt__"] is not None else sys_row["__system_reporting_dt__"]
...
def build_main_output(
    ...
    for i in range(len(gate_df)):
        ...
        out["Base_Gate_In_Date"] = _format_date(g["__base_gate_in_dt__"].date()) if g["__base_gate_in_dt__"] else None
        out["Base_Gate_In_Time"] = _format_time(g["__base_gate_in_dt__"].time().replace(microsecond=0)) if g["__base_gate_in_dt__"] else None
        ...
        base_gate_in_dt = g["__base_gate_in_dt__"]
        gate_out_dt = g["__workshop_out_dt__"]
        ...
        if original_tat is not None and original_tat < 0:
            toggled_time = _toggle_ampm(gate_out_dt.time()) if gate_out_dt else None
            toggled_dt = datetime.combine(gate_out_dt.date(), toggled_time) if gate_out_dt and toggled_time else None
            toggled_tat = _safe_hours(toggled_dt - base_gate_in_dt) if toggled_dt and base_gate_in_dt else None
            ...
            elif matched_sys and matched_sys["__system_bill_dt__"] is not None:
                corrected_gate_out_dt = matched_sys["__system_bill_dt__"]
                tat_corrected = True
        ...
        out["Corrected_Gate_Out_Date"] = _format_date(corrected_gate_out_dt.date()) if corrected_gate_out_dt else None
        out["Corrected_Gate_Out_Time"] = _format_time(corrected_gate_out_dt.time().replace(microsecond=0)) if corrected_gate_out_dt else None
